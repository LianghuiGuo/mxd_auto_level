...
class MainWindow(QMainWindow):
    ...
    def __init__(self, controller=None):
        ...
        # Populate UI inputs with cfg_base defaults. This is the safety net
        # that guarantees no numeric QLineEdit stays empty, even if the
        # following load_ui_state() encounters a missing/erroneous config.
    ...
    def update_cfg_from_main_ui(self):
        ...
        All numeric inputs are parsed via safe_int / safe_float with fallback
        to cfg_base defaults so empty or malformed fields never crash.
        '''
        # Bot control gbox
        ...
        if self.attack_mode.currentText() == "Basic":
            ...
            base = self.cfg_base["directional_attack"]
            self.cfg["directional_attack"]["range_x"] = safe_int(
                self.attack_range_x.text(), base["range_x"])
            self.cfg["directional_attack"]["range_y"] = safe_int(
                self.attack_range_y.text(), base["range_y"])
            self.cfg["directional_attack"]["cooldown"] = safe_float(
                self.attack_cooldown.text(), base["cooldown"])
            ...
            base = self.cfg_base["aoe_skill"]
            self.cfg["aoe_skill"]["range_x"] = safe_int(
                self.attack_range_x.text(), base["range_x"])
            self.cfg["aoe_skill"]["range_y"] = safe_int(
                self.attack_range_y.text(), base["range_y"])
            self.cfg["aoe_skill"]["cooldown"] = safe_float(
                self.attack_cooldown.text(), base["cooldown"])
        else:
        ...
        if self.checkbox_auto_add_hp.isChecked():
            base_hp = self.cfg_base["health_monitor"]["add_hp_percent"]
            self.cfg["health_monitor"]["add_hp_percent"] = safe_int(
                self.add_hp_percent.text(), base_hp)
        ...
        if self.checkbox_auto_add_mp.isChecked():
            base_mp = self.cfg_base["health_monitor"]["add_mp_percent"]
            self.cfg["health_monitor"]["add_mp_percent"] = safe_int(
                self.add_mp_percent.text(), base_mp)
