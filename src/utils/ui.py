...
def safe_int(input_str, default=0):
    """
    Safely convert a string to int.
    Returns ``default`` if the input is empty, None, or not a valid integer.
    """
    ...
    try:
        ...
        try:
            # Allow float-like string such as "3.0"
            return int(float(s))
        except (ValueError, TypeError):
            logger.warning(f"[safe_int] Cannot parse '{input_str}', fallback to default {default}.")
            return default


def safe_float(input_str, default=0.0):
    """
    Safely convert a string to float.
    Returns ``default`` if the input is empty, None, or not a valid float.
    """
    if input_str is None:
        return default
    s = str(input_str).strip()
    ...
    try:
        ...
        logger.warning(f"[safe_float] Cannot parse '{input_str}', fallback to default {default}.")
        return default
